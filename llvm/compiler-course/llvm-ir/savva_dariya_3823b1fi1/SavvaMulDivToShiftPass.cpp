...
class SavvaMulDivToShiftPass final : public llvm::PassInfoMixin<SavvaMulDivToShiftPass> {
public:
    ...
    for (auto &BB : F) {
      for (auto &I : llvm::make_early_inc_range(BB)) {
        ...
        if (Opcode != llvm::Instruction::Mul && 
            ...
            Opcode != llvm::Instruction::UDiv) continue;
        ...
        if (Opcode == llvm::Instruction::Mul) {
          if (auto *C = llvm::dyn_cast<llvm::ConstantInt>(BinOp->getOperand(1))) {
            ConstOp = C; VarOp = BinOp->getOperand(0);
          } else if (auto *C = llvm::dyn_cast<llvm::ConstantInt>(BinOp->getOperand(0))) {
            ConstOp = C; VarOp = BinOp->getOperand(1);
          }
        } 
        ...
        if (!ConstOp || !VarOp) continue;
        
        const auto &AP = ConstOp->getValue();
        if (!AP.isPowerOf2() || !AP.isStrictlyPositive()) continue;
        ...
        switch (Opcode) {
          ...
          case llvm::Instruction::SDiv:
            Result = createSignedShift(Builder, VarOp, ShiftAmount, BinOp->getType());
            break;
        }
...
private:
  llvm::Value* createSignedShift(llvm::IRBuilder<> &Builder,
                                 llvm::Value *LHS,
                                 llvm::Value *Shift,
                                 llvm::Type *Ty) {
    ...
    auto *Correction = Builder.CreateSelect(IsNegative, CorrectionMask, Zero, "savva.corr");
