...
class ThrowAndCallVisitor : public clang::RecursiveASTVisitor<ThrowAndCallVisitor> {
public:
  bool hasThrow = false;
  ...
  void HandleTranslationUnit(clang::ASTContext &context) override {
    ...
    for (auto decl : TU->decls()) {
      if (auto func = llvm::dyn_cast<clang::FunctionDecl>(decl)) {
        ...
        if (!func->hasBody()) continue;                    // Нужно тело
        if (func->isImplicit()) continue;                  // Неявные пропускаем
        if (func->isDeleted() || func->isDefaulted()) continue; // Специальные
        if (context.getSourceManager().isInSystemHeader(func->getLocation())) continue;
        if (!func->getType()->isFunctionProtoType()) continue; // Нужен прототип
    ...
    std::unordered_map<const clang::FunctionDecl*,
                       std::vector<const clang::FunctionDecl*>> callGraph;
    ...
    while (changed) {
      ...
      for (auto &entry : callGraph) {
        ...
        if (functionThrows[caller]) continue;
    ...
    for (auto func : functions) {
      ...
      if (functionThrows[func]) continue;
      
      // Пропускаем функции, уже имеющие noexcept
      if (func->getExceptionSpecType() == clang::EST_BasicNoexcept) continue;
...
private:
  ...
  void insertNoexcept(clang::FunctionDecl *func) {
    ...
    clang::QualType newType = m_context->getFunctionType(ftp->getReturnType(), ftp->getParamTypes(), epi);
